...
task("deploy", "Deploy DataDAOBounty contract")
    ...
    .setAction(async ( { admins, membershipnftaddress, logs }, { ethers }) => {
        ...
        if (!membershipnftaddress) {
            ...
            console.log("Deploying DataDAOBounty Contract")

            const DataDAOBounty = await ethers.getContractFactory("DataDAOBounty")

            const DataDAOBountyContract = await DataDAOBounty.deploy(adminsList, membershipnftaddress, {
                gasLimit: 1000000000,
                maxPriorityFeePerGas: priorityFee
            })
    
            await DataDAOBountyContract.deployed()

            if (logs) {
                console.info(`DataDAOBounty contract has been deployed to: ${DataDAOBountyContract.address}`)
            }

            return DataDAOBountyContract
