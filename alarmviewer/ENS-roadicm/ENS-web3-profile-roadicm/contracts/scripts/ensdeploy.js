...
async function main() {
  ...
  const registrar = await  FIFSRegistrar.deploy(ens.address, utils.namehash(tld));
  ...
  console.log('Deployed', { reverse: reverseRegistrar.address, fifs: registrar.address, resolver: resolver.address, registry: ens.address })
};

async function setupResolver(ens, resolver, accounts) {
  const resolverNode = utils.namehash("resolver");
...
async function setupReverseRegistrar(ens, registrar, reverseRegistrar, accounts) {
  ...
  await ens.setSubnodeOwner(utils.namehash("reverse"), labelhash("addr"), reverseRegistrar.address);
}
