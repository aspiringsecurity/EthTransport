...
.factory('CloudService', function($http){
         ...
         $http.get("http://aspiringapps.com/webapp",{params:{action:"getInapp",appname:"Driving Journal"}}).
         success(function(response) {
                 ...
                 var message = {action:"update",appname:"Driving Journal"};
                 $http({url:"http://aspiringapps.com/webapp",method:"POST",params:message}).
                 success(function(responses){
                         var results = responses.result;
                         ////alert("sencond:"+result);
                         if(results == "ok"){
                         var productList=[{"Feature": "save","Id": "2015djCloud","Purchase":"Yes","Consumed":0,"Own":5}];
                 ...
                 var items;
                 if(result.own == 0){
                 items=[{"Feature": "save","Id": "2015djCloud","Purchase":"No","Consumed":result.consumed,"Own":result.own}];
                 }
                 else{
                 items=[{"Feature": "save","Id": "2015djCloud","Purchase":"Yes","Consumed":result.consumed,"Own":result.own}];
                 }
         ...
         $http.get("http://aspiringapps.com/restore",{params:{action:"getInapp",appname:"Driving Journal"}}).
         success(function(response) {
                 // this callback will be called asynchronously
                 // when the response is available
                 var result = response.result;
...
.factory('ModalService',function($ionicModal, $timeout, CloudService){
         ...
         var message = {action:"login",uuid:uuid,password:pass,deviceId:device.uuid,appname:"Driving Journal"};
         ...
         CloudService.request(message).then(function(response){
                                            ...
                                            var message = {action:"getInapp",appname:"Driving Journal"};
         ...
         var message={action:"register",uuid:uuid,password:pass,appname:"Driving Journal"};
...
.factory('MoreAppService', function(){
         ...
         appbundles = [{name: 'Business Suite', desc: 'Get a comprehensive view of your business',
                       ...
                       {name: 'Business Ledger Suite', desc: 'Managing your finances and do financial book keeping',
