...
console.log("coord "+wval.ecoord);
//clearArray.push(wval.ecoord);

if(!window.localStorage.getItem("cellArray")){
//alert("no");
var clearArray2=new Array();
clearArray2.push(wval.ecoord);
arr.push({"sheet":acsheet, "editedcells":clearArray2});
console.log("updating no "+acsheet+" cells coord "+wval.ecoord );
window.localStorage.setItem("cellArray",JSON.stringify(arr));
console.log("array is "+JSON.stringify(arr));
}

else if(window.localStorage.getItem("cellArray")){

var data=[];
data=JSON.parse(window.localStorage.getItem("cellArray"));
for(var i=0;i<data.length;i++){
if(data[i].sheet==acsheet){
ctr++;
var clearArray=data[i].editedcells;
//alert("both");

clearArray.push(wval.ecoord);
data[i].editedcells=clearArray;
//alert(data[i].editedcells);
//                                         data[i].editedcells=clearArray;
window.localStorage.setItem("cellArray",JSON.stringify(data));
console.log("updating both "+acsheet+" cells coord "+wval.ecoord );
console.log("array length is "+data.length);
console.log("stored "+JSON.stringify(data));
}
}
if(ctr==0){
var clearArray1=new Array();
// alert("object exists but entry does not");
clearArray1.push(wval.ecoord);
data.push({"sheet":acsheet, "editedcells":clearArray1});
window.localStorage.setItem("cellArray",JSON.stringify(data));
console.log("updating 1 "+acsheet+" cells coord "+wval.ecoord );
...
else if(device=="iPad" || device=="default"){
